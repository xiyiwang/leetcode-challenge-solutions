"""
LeetCode Challenge: Find Duplicate File in System (2021-05-18)
...
the file system in terms of their paths. You may return the answer in any order.
...
- You may assume each given directory info represents a unique directory. A single 
  blank space separates the directory path and file info.

Follow up:
- Imagine you are given a real file system, how will you search files? DFS or BFS?
  -> BFS for running in parallel
- If the file content is very large (GB level), how will you modify your solution?
  -> 1. Compare file size
  -> 2. Compare fast hash of the content (e.g. use SHA256)
...
class Solution:
    # runtime: 88 ms (beats 61.55%)
    def findDuplicate(self, paths: list) -> list:
        D = dict()

        for path in paths:
            path = path.split(" ")
            dir, fns = path[0], path[1:]
            for fn in fns:
                fn = fn.split("(")
                p = dir + "/" + fn[0]
                content = fn[1][:-1]
                if content not in D: D[content] = [p]
                else: D[content].append(p)

        return [_ for _ in D.values() if len(_) > 1]
